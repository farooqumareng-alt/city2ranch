...
/**
 * Admin counterpart to ProfileForm — same fields, same validation
 * (profileUpdateSchema), bound to updateCustomerProfileAsAdmin instead
 * of the customer's own updateProfile. Email is deliberately absent —
 * see that action's own doc comment for why login identity isn't
 * something this form touches.
 */
    ...
    <form action={formAction} className="flex flex-col gap-10">
      ...
      <fieldset className="flex flex-col gap-4">
        ...
        <p className="font-sans text-xs text-charcoal/60">
          Email isn&apos;t editable here — it&apos;s the customer&apos;s own sign-in identity.
        </p>
        <div className="grid gap-4 sm:grid-cols-2">
          <TextField name="name" label="Full name" defaultValue={profile.name ?? ""} error={fieldErrors?.name} />
          <TextField
            name="phone"
            type="tel"
            label="Phone"
      ...
      <fieldset className="flex flex-col gap-4">
        <legend className="font-serif text-lg text-navy-deep">Default Delivery Address</legend>
        <p className="font-sans text-xs text-charcoal/60">Pre-fills new City Pickup orders for this customer.</p>
        <div className="grid gap-4 sm:grid-cols-2">
          <TextField
